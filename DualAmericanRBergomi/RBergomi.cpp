...
Vector RBergomi::generateEuler() {

	return std::get<0>(generate_Euler_SV());
}

void RBergomi::operator()(Vector& S, Vector& X, char method) {
	if(method == 's')
		S = generate();
	else
		S = generateEuler();
...
std::pair<Vector, Vector> RBergomi::generate_SV() {
	...
	// define the renormalisation constant; Modification with 1/32 gives best errors so far (K=1), but this depends on N
	// Hence, there is a more fundamental error here...
	const double Cepsilon = sqrt(2 * H) / ((H + 0.5) * (H + 1.5)) * pow(2.0, (log2(rep->getN()-0) / (1*rep->getK())) * (0.5 - H));
	//const double Cepsilon = sqrt(2 * H) / ((H + 0.5) * (H + 1.5)) * pow(2.0, rep->getN() / rep->getK() * (0.5 - H));
...
std::pair<Vector, Vector> RBergomi::generate_Euler_SV() {
	// generate random numbers
	rep->generate();
	Vector Wtilde = rep->WtildeEuler();
	Vector dB1 = rep->dB(0);
	Vector dB2 = rep->dB(1);
	Vector S(N + 1);
	Vector X(N + 1); // log-price
	...
	for (int n = 0; n < N - 1; ++n) {
		v[n + 1] = xi0((n + 1) * ds)
				* exp(
						eta * Wtilde[n + 1]
								- 0.5 * eta * eta * pow((n + 1) * ds, 2.0 * H));
		X[n + 1] = X[n] + (r - 0.5 * v[n]) * ds
				+ sqrt(v[n]) * (rho * dB1[n] + rhobar * dB2[n]);
	}
	X[N] = X[N-1] + (r - 0.5 * v[N - 1]) * ds
			+ sqrt(v[N - 1]) * (rho * dB1[N - 1] + rhobar * dB2[N - 1]);
	// S = exp(X)
	std::transform(X.begin(), X.end(), S.begin(),
			[](double x) -> double {return exp(x);});
