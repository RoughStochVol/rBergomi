...
BSImpliedVolTehranchi <- function(S0, K, T, r, C)
{
  f <- S0 * exp(r*T)
  x <- C/f
  k <- log(K/f)
  # sigmaL and sigmaH are Tehranchi upper and lower bounds
  sigmaL <- -2 * qnorm((1-x)/(1+exp(k)*(k<0)+(k >=0)))
  sigmaH <- - 2 * qnorm((1-x)/(1+exp(k)))
...
BSImpliedVolPutTehranchi <- function(S0, K, T, r, P)
{
  f <- S0 * exp(r*T)
  x <- P/f
  k <- log(K/f)
...
BSImpliedVolMRS <- function(S0, K, T, r, C)
{
  nK <- length(K)
  sig0 <- 0.2*sqrt(T)
  k <- log(K/S0)
  x0 <- BSFormula(1, exp(k), 1, r=0, sig0)
  dplus <- -k/sig0+sig0/2
  np.dplus <- dnorm(dplus)
  sigp0 <- 1/np.dplus
  sigpp0 <- sigp0^2 /sig0^3 *( sig0^4/2 - k^2) 
  x <- C/S0
  
  while (mean(abs((x - x0))) > 1e-8)
  {
    sig0 <- sig0 + sigp0 *(x-x0) + sigpp0 * (x-x0)^2/2
    x0 <- BSFormula(1,exp(k), 1, r=0, sig0)
    dplus <- -k/sig0+sig0/2
    np.dplus <- dnorm(dplus)
    sigp0 <- 1/np.dplus
