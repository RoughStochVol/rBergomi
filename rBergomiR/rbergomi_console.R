...
rBergomi.pricer2 <- function(xi, H, eta, rho, T, K, N, M, num.jobs, var.red = FALSE){
  ...
  if(Sys.info()[1] == "Darwin")
    exec.name <- paste(path, "Release/ML_console_ST", sep = "")
  else
    exec.name <- paste(path, "ML_console_ST", sep = "")
  
  arg.list <- list(finf(N), finf(M), path, 
                   paste(unlist(out.names, use.names = FALSE), sep = " ", collapse = " "), 
                   paste(unlist(in.names, , use.names = FALSE), sep = " ", collapse = " "), 
                   method)
  
  args <- paste(" ::: ", paste(arg.list, collapse = " ::: "), sep = "")
 
  sys.command <- paste("parallel --link", exec.name, args, sep = " ")
  system(sys.command)
...
rBergomi.pricer2.simulate <- function(xi, H, eta, rho, T, 
                                      K, N, M, num.jobs, var.red = FALSE){
  ## Convert number to string containing first 10 digits
  f10 <- function(x) formatC(x, digits=10, format="f")
